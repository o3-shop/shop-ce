...
- New features:
    ...
    - O3-Shop console, which allows to register custom commands for modules and for components via `services.yaml`.
...
- `\OxidEsales\EshopCommunity\Core\Email::$Version` This fixes a missing deprecation in PHPMailer, which is the parent
  ...
  of O3-Shop CE as PHPMailer 5 will be no longer maintained. Please note, that there are some breaking changes for
...
- `sO3SHOPPHP` parameter in `config.inc.php`
...
- CSS adapted in O3-Shop Setup to reflect new design, extracted styles to separate file `Setup/out/src/main.css`
...
- In `oxshops` table field - `OXEDITION` is deprecated. To retrieve O3-Shop edition
...
- ddoe/wysiwyg-editor-module was added as requirement of O3-Shop Community Edition in composer.json
...
- Database migrations and views regeneration is operating system independent which makes O3-Shop installable on Windows.
...
### Removed
- Azure theme was extracted from the O3-Shop CE repository to [separate repository](https://github.com/OXID-eSales/azure-theme).
...
- Database columns were changed due to unification of O3-Shop editions.
...
- PayPal module, which is compatible with O3-Shop, has been added to the compilation.
