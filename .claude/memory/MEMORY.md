...
- [Adding a system requirement](reference_adding-a-system-requirement.md) — setup screen + admin System Health are data-driven: 3 touch points (getRequiredModules + `check<CamelCase>()` derived name + `MOD_*`/`SYSREQ_*` lang keys), no template edits; status 0 blocks install
