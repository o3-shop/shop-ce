...
- [ParaTest parallel runner (opt-in)](paratest-parallel-runner.md) — `./docker.sh test --parallel [-p N]`; per-worker DB/compile/config isolation via TEST_TOKEN + env override; runner self-heals stripped testing-lib files; still opt-in (sequential default) due to ~1-6 flaky order-coupled tests; ~2.3x faster
- [Self-hosted runner for the 8.2 coverage leg](ci-self-hosted-runner-8.2-coverage.md) — CI routes only PHP 8.2 (coverage) to a self-hosted box for trusted non-fork pushes; the 8-step environment gauntlet (Docker/sudo/mysql-client/SSL×2/paths/extensions/wave); tmpfs DB + `-p 20` + CLI opcache = ~2:49 leg (was 16min); workers plateau + flakiness ceiling; ~2:30 needs DB-isolation rearchitecture (install/clone won't help — DB-bound)
...
- [!] [Committed lock pinned to PHP 7.4 floor — never cache vendor/](ci-lockless-composer-vendor-cache.md) — composer.lock is COMMITTED (Dependabot) and MUST be pinned via config.platform.php=7.4.33 or `composer install` fails fast (exit 2) on the 7.4/8.0/8.1 matrix legs. Still never cache vendor/; cache ~/.cache/composer only.
...
- [Symfony 3.4->5.4 migration (b-2.0)](symfony5-migration-b2.0.md) — unit suite GREEN after fixing 5 categories (private services, ServicesYamlValidator, Command::execute int, dispatch arg order); remaining cleanup is PHPUnit-10/Integration-only; local test-env gotchas (testing-lib file stripping, db alias, --skip-ssl)
- [!] [Symfony 5.4: services private by default](symfony5-private-services-container-get.md) — container->get(X) throws ServiceNotFoundException "removed or inlined" unless X is public:true; the 3.4->5.4 migration left ContextInterface private (broke UserTest). Mark services fetched by id public; clear container_cache.php after.
