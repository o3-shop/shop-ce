...
- Add three shop-level configuration switches in admin:
  ...
  - "Notify operator by email" (default: **on** for both fresh installs and upgrades) with optional recipient address. **Why on by default:** if it were off, the operator could go indefinitely without learning that revocations are arriving — submissions would just pile up in the admin module. That risks missed legal deadlines and unhappy consumers. The recipient address (`sRevocationOperatorEmail`) is **optional**: when empty, the email is sent to the shop's existing order-confirmation address (`oxshops.oxorderemail`), which every functioning shop already has configured. Operators who want a different recipient set it explicitly. The boolean+string two-row pattern matches O3 convention so operators can mute notifications temporarily without losing their configured recipient address.
...
- `migration/` — new Doctrine migration creating the `o3revocation` table and seeding one empty + inactive `oxcontents` row per active shop language for the operator notice snippet (idempotent — never overwrites existing operator content). The migration does **not** seed `oxconfig` rows (see "Setup" below).
- `Setup/Sql/initial_data.sql` *(loaded only on fresh install via `source/Setup/Controller.php:639`)* — add one extra `INSERT INTO oxconfig` for `blShowRevocationForm = 1`, alongside the nine config rows already seeded there. Fresh installs come up with the legally-safe default on; upgrades inherit the absent-row → off behaviour from code (see design D5). No `shop-demodata-*` repo touched.
...
**APIs / Routes**
- New public routes: `?cl=revocation` (form), `?cl=revocation&fnc=submit` (submit), `?cl=revocation&fnc=confirm` (confirmation step / receipt page)
...
- New `oxcontents` row(s): one per active shop language, ident `o3_revocation_notice`, seeded empty by the migration. Operator-editable thereafter — the migration must NOT overwrite existing content if the row already exists (idempotent seed).
