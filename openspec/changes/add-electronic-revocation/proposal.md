...
- Add three shop-level configuration switches in admin:
  ...
  - "Notify operator by email" (default: off) with recipient address
...
- `migration/` — new migration creating the `o3revocation` table (or similar), seeding the four new shop config rows in `oxconfig` (three switches + operator email recipient), and seeding one empty + inactive `oxcontents` row per active shop language for the operator notice snippet (idempotent — never overwrite existing operator content)
...
**Database**
- New table for revocation submissions (id, name, order identifier, contact channel, optional free-text, submitted_at, ip/user-agent for fraud forensics, language, shop_id)
