...
- [ ] 17.2 Resolve any issues uncovered by the dry-run (missing release branches, malformed `.next-bump` files, etc.)
- [ ] 17.3 Run `bin/release --from v1.6.0 --to v1.6.1-RC1` for real — depends on §15 (live wiring) and §16 (zero-config discovery). This is the first machine-driven release and ships this entire change.
- [ ] 17.4 Manually publish the draft GitHub releases per repo and the aggregated o3-shop draft
- [ ] 17.5 No merge-back PRs are auto-opened (RC1 is pre-release); merge-back PRs land with the eventual v1.6.1 final cut
- [ ] 17.6 Run Section 18 verification on the produced v1.6.1-RC1 artifact
- [ ] 17.7 Capture lessons learned in `.claude/memory/` (per the repo's finish protocol)
...
- [ ] 18.1 Verify a fresh `composer install` of `o3-shop v1.6.1-RC1` produces a working shop with `ShopVersion::getVersion() === "v1.6.1-RC1"` (folds in former §12.2 — composer-install integration check against the post-fold-in `o3-shop/composer.json`)
- [ ] 18.2 Smoke-test the admin UI: confirm the version display shows `v1.6.1-RC1`
- [ ] 18.3 Verify `o3-shop/composer.json` at `v1.6.1-RC1` is post-fold-in (no `o3-shop/shop-metapackage-ce` in `require`, `replace: oxid-esales/oxideshop-metapackage-ce` present)
- [ ] 18.4 Verify the v1.6.1-RC1 dist archive does not contain `.next-bump` (archive.exclude works end-to-end) (folds in former §12.3 — `.next-bump` archive-exclude check, exercised end-to-end against the real cut)
