# release-graph-derivation Specification

## Purpose

Defines how `bin/release` derives the per-release dependency graph from
two inputs (`--from` snapshot and the live release-branch state),
including how it picks per-repo versions, when it cuts new tags, and
how it updates downstream constraints. The graph is the algorithmic
core of the release procedure — every other capability consumes its
output.
...
If `o3-shop/composer.json` at the `--from` tag still requires `o3-shop/shop-metapackage-ce` (the snapshot predates the metapackage fold-in), the CLI SHALL recurse one level into the metapackage's
...
If the dep walk discovers a cycle (e.g. A requires B requires A), the CLI SHALL abort with a diagnostic listing the cycle.
...
When `--to` is a final shop release (no `-rc`/`-alpha`/`-beta` suffix), the CLI SHALL reject pre-release candidate tags. When `--to` is a pre-release, the CLI SHALL accept either.
...
When the CLI cuts a new tag on any repo other than `shop-ce`, the new tag SHALL be a bump from the repo's current latest tag, on the repo's existing version line. The bump level is resolved with this precedence (first match wins):
...
When the CLI uses a `.next-bump` file's value to compute a new tag, it SHALL delete the file in the same commit it cuts the tag from, so the released tag's tree does not contain the file.
...
For every spot in the dep tree where a candidate is pinned, the CLI SHALL update the constraint **only when** the existing constraint does not already satisfy the chosen version. Exact pins SHALL be replaced with the chosen tag verbatim. Flexible constraints (caret, tilde, range) SHALL be widened only when the chosen version does not satisfy them.
