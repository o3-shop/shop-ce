# release-notes-aggregation Specification

## Purpose

Defines how cross-repo release notes are produced for an o3-shop
release. The CLI delegates per-repo note generation to GitHub's
generate-notes API and aggregates the results into a single body
attached to the o3-shop draft release. Per-repo draft releases keep
their own notes; the aggregation is additive.

## Requirements

### Requirement: Per-repo notes generated via GitHub generate-notes API

For every release-eligible repo where the chosen version differs from `from_pin[repo]`, the CLI SHALL request release notes from GitHub's generate-notes endpoint:
