# admin-update-check Specification

## Purpose

Provide a rich update check in the O3-Shop admin backend that reports both the core shop version status and the update status of installed modules. The check collects the current shop version, shop domain, and configured module IDs/versions, sends them to the O3-Shop update endpoint, and renders a structured result (core notice plus per-module rows) in the admin UI. Both an automatic check on admin login (gated by `blCheckForUpdates`) and a manual "Check for updates" trigger are supported.
