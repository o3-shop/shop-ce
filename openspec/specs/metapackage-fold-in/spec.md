# metapackage-fold-in Specification

## Purpose

Defines how `o3-shop/shop-metapackage-ce` is folded into `o3-shop/o3-shop`
so that the umbrella project — not a separate metapackage — owns the
top-level dependency graph. Captures which deps move, which are dropped,
which are preserved, and the post-fold-in archival of the legacy repo.
...
The `o3-shop/shop-metapackage-ce` GitHub repo SHALL be archived after
the fold-in is applied and merged. A final tag SHALL pin its current
state. Its README SHALL be updated to point new consumers at
`o3-shop/o3-shop`.
