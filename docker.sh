...
run_full_test_with_coverage() {
  ...
  echo "---------------------------"
  # Coverage runs SEQUENTIALLY (single process, pcov driver) — parallel coverage
  # only contends on the shared DB / FUSE mount and is slower. Extra args are
...
case "$1" in
    ...
    *)
        ...
        echo ""
        echo "  test         Run the unit tests (parallel, -p4 by default)"
        echo "  test-all     Run php-cs-fixer, then the full test suite (parallel)"
        echo "  test-all-coverage  Run php-cs-fixer, then the full suite with a coverage report"
        echo "                     (coverage runs sequentially, pcov driver)"
        ...
        echo "  -p N             ParaTest worker count (default 4). Each worker"
        echo "                   gets its own isolated database, compile dir and"
        echo "                   project-config dir."
        echo "  --sequential     Force the single-process runner (default is parallel)."
        echo "  --coverage       Coverage reports (clover, html, junit); runs"
        echo "                   sequentially (pcov)."
        echo "  --fast           Skip shop install, call phpunit directly (subset iteration)."
        echo "  --all-failures   Don't stop at the first failure — run the full"
        echo "                   suite and collect every failure in one pass."
        echo ""
        echo "Examples:"
        echo "  $0 start"
        echo "  $0 test                                  # full suite, parallel -p4"
        echo "  $0 test -p 6                              # full suite, parallel -p6"
        echo "  $0 test --sequential                      # full suite, one process"
        echo "  $0 test --fast tests/Unit/Core/ConfigTest.php"
        echo "  $0 test --coverage                        # full suite + coverage"
        echo "  $0 test-all"
        echo "  $0 quarantine"
        exit
        ;;
