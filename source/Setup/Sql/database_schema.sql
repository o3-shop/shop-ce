...
create index ROLETYPE_IDX
    on o3rightsroleselements (OBJECTID, TYPE);
create index OBJECT_IDX
    on o3rightsroleselements (OBJECTID);
...
CREATE TABLE `oxarticles` (
  ...
  `O3GUARANTEEDURATIONMONTHS` int NOT NULL default '0' COMMENT 'EmpCo commercial durability guarantee duration in months, as communicated by the producer. 0 = no durability guarantee communicated (default, not a legal statement). Stored in months (not years) so non-integer-year guarantees can be represented. The harmonised durability-guarantee label (Implementing Regulation (EU) 2025/1960) is only mandatory when this value is strictly greater than 24 (more than two years, Art. 22a Directive (EU) 2024/825). The trader has no duty to research this value themselves.',
  `O3GUARANTEEGUARANTOR` varchar(255) NOT NULL default '' COMMENT 'Guarantor/brand name to display on the durability-guarantee label. Empty falls back to the linked oxmanufacturers title at render time; if that is also empty the label cannot render.',
  `O3GUARANTEEMODEL` varchar(255) NOT NULL default '' COMMENT 'Model identifier to display on the durability-guarantee label (a mandatory variable component per Annex II, Implementing Regulation (EU) 2025/1960). Empty falls back to OXARTNUM at render time.',
  `O3GUARANTEECONDITIONS` text COMMENT 'Guarantee conditions text/URL for the pre-existing § 479 BGB guarantee information duties (guarantor address, territorial scope, conditions), triggered once the durability label is displayed.',
