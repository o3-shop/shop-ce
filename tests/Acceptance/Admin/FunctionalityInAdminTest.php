...
class FunctionalityInAdminTest extends AdminTestCase
{
    ...
    public function testEditingOrdersMain()
    {
        ...
        $this->callShopSC('oxOrder', 'save', 'testorder7', ['OXFOLDER' => 'ORDERFOLDER_FINISHED']);
        $this->loginAdmin('Administer Orders', 'Orders');
        $this->selectAndWait('folder', 'label=Finished');
    ...
    public function testLoginToAdminInOtherLang()
    {
        ...
        $this->loginAdmin();
        $this->waitForText('Welcome to the O3-Shop Admin');
        $this->frame('navigation');
        $this->checkForErrors();
        ...
        $this->loginAdmin(null, null, false, 'admin@my-o3-shop.com', 'admin0303', 'Deutsch');
        $this->waitForText('Willkommen im O3-Shop Administrationsbereich');
        $this->checkForErrors();
        $this->frame('navigation');
        $this->checkForErrors();
        ...
        $this->openNewWindow($shopUrl . 'admin');
        $this->open($url);
        $this->type('usr', 'admin@my-o3-shop.com');
    ...
    public function testHelpPopupsInAdmin()
    {
        ...
        $this->assertEquals('Close Non-productive Shop mode is intended', substr($this->clearString($this->getText('helpPanel')), 0, 43));
    ...
    public function testEditShopName()
    {
        ...
        $this->assertEquals("O3-Shop $shopVersionNumber", $this->getValue('editval[oxshops__oxname]'));
    }
    ...
    public function testEditShopSave()
    {
        ...
        $this->assertEquals('www.my-o3-shop.com', $this->getValue('editval[oxshops__oxurl]'));
        ...
        $this->type('editval[oxshops__oxname]', 'O3-Shop šÄßüл');
        ...
        $this->assertEquals('Ihr Passwort im O3-Shop', $this->getValue('editval[oxshops__oxforgotpwdsubject]'));
        ...
        $this->type('editval[oxshops__oxregistersubject]', 'Vielen Dank fur Ihre Registrierung im O3-Shop1_šÄßüл');
        $this->type('editval[oxshops__oxforgotpwdsubject]', 'Ihr Passwort im O3-Shop1_šÄßüл');
        ...
        $this->assertEquals('O3-Shop šÄßüл', $this->getValue('editval[oxshops__oxname]'));
        $this->assertEquals('localhost', $this->getValue('editval[oxshops__oxsmtp]'));
        ...
        $this->assertEquals('Vielen Dank fur Ihre Registrierung im O3-Shop1_šÄßüл', $this->getValue('editval[oxshops__oxregistersubject]'));
        $this->assertEquals('Ihr Passwort im O3-Shop1_šÄßüл', $this->getValue('editval[oxshops__oxforgotpwdsubject]'));
        ...
        $this->assertEquals($this->getSelectedLabel('subjlang'), 'English');
        $this->assertEquals('Your order at O3-Shop', $this->getValue('editval[oxshops__oxordersubject]'));
        $this->type('editval[oxshops__oxordersubject]', 'Your order from O3-Shop1');
        $this->type('editval[oxshops__oxregistersubject]', 'Thank you for your registration in O3-Shop1');
        $this->type('editval[oxshops__oxforgotpwdsubject]', 'Your O3-Shop password1');
        $this->type('editval[oxshops__oxsendednowsubject]', 'Your OXID eSales Order has been shipped1');
        $this->type('oxsmtppwd', '-');
        $this->clickAndWaitFrame('save', 'list');

        $this->assertEquals('Your order from O3-Shop1', $this->getValue('editval[oxshops__oxordersubject]'));
        $this->assertEquals('Thank you for your registration in O3-Shop1', $this->getValue('editval[oxshops__oxregistersubject]'));
        $this->assertEquals('Your O3-Shop password1', $this->getValue('editval[oxshops__oxforgotpwdsubject]'));
    ...
    private function updateSubshopOrders()
    {
        ...
        if ($testConfig->isSubShop()) {
            ...
            foreach ($orders as $order) {
                $this->callShopSC('oxOrder', 'save', $order[0], ['oxshopid' => $testConfig->getShopId()], null, 1);
            }
        }
    }
