...
class ViewTest extends \OxidTestCase
{
    ...
    {
        $oSetupView = $this->getMock(\OxidEsales\EshopCommunity\Setup\View::class, ['getText']);
    ...
    {
        $oSetupView = new View();
        $this->assertEquals([], $oSetupView->getMessages());
    }
    ...
    public function testSetMessage()
    {
        ...
        $this->assertEquals(['msg1'], $oSetupView->getMessages());
        $oSetupView->setMessage('msg2');
        $this->assertEquals(['msg1', 'msg2'], $oSetupView->getMessages());
        $oSetupView->setMessage('msg3', true);
        $this->assertEquals(['msg3'], $oSetupView->getMessages());
    }
    ...
    {
        $oInst = $this->getMock('Language', ['getText']);
        $oInst->expects($this->once())->method('getText')->with($this->equalTo('testId'));

        $oSetupView = $this->getMock(\OxidEsales\EshopCommunity\Setup\View::class, ['getInstance']);
    ...
    {
        $oInst = $this->getMock('Setup', ['getStep']);
    ...
    {
        $oInst = $this->getMock('Setup', ['getNextStep']);
    ...
    {
        $oInst = $this->getMock('Setup', ['getCurrentStep']);
    ...
    {
        $oInst = $this->getMock('Setup', ['getSteps']);
    ...
    public function testIsDeletedsetup(): void
    {
        ...
        $oInst2 = $this->getMock('Utilities', ['removeDir']);
        $oInst2->expects($this->exactly(3))
            ->method('removeDir')
            ->withConsecutive(
                [$this->equalTo($sPath . 'out/pictures/generated'), $this->equalTo(true)],
                [$this->equalTo($sPath . 'out/pictures/master'), $this->equalTo(true), $this->equalTo(1), $this->equalTo(['nopic.jpg'])],
    ...
    {
        $oInst = $this->getMock(SetupSession::class, ['getSid'], [], '', false);
